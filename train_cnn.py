...
def load_char_map(path='python/artifacts/char_map_en.json'):
    with open(path, encoding='utf-8') as f:
        return json.load(f)

def load_labels(path='python/labels_en.txt'):
    with open(path, encoding='utf-8') as f:
...
def main(args):
    ...
    # Load training data
    df = pd.read_csv('python/data/train_en.csv')
    ...
    model_file = 'models/cnn_en.keras'   # <-- save as .keras for Keras 3
    model.save(model_file, include_optimizer=False)

    # Save metadata
    meta = {'max_len': max_len, 'emb_dim': args.emb_dim}
    with open('models/meta_en.json', 'w', encoding='utf-8') as f:
        json.dump(meta, f)
